...
- The stack space is not that unlimited (especially when you need to create a large array).
  ...
  as address-needed(maybe not in semantic check phase) and reduce some useless store-n-load pairs.
...
One might use "RegValue visit(node)" pattern to more precisely identify whether expressions returns lvalues.
