...
std::string remove_underscore(StringT str) {
  static std::string res; res.clear();
  for(const char c: str) if(c != '_') res += c;
  return res;
}

std::uint64_t resolve_integer(StringT lexeme) {
...
bool ident_not_reserved(StringT ident) {
  ...
  while(infix_precedence.contains(_ast_ctx->current().type) &&
    ...
    case TokenType::kDot: {
      ...
      if(CHECK_TOKEN(kIntegerLiteral)) {
        ...
        lft = std::make_unique<TupleIndexingExpression>(std::move(lft), index);
        break;
      }
      ...
      if(CHECK_TOKEN(kLParenthesis)) {
        ...
        MATCH_TOKEN(kRParenthesis);
        lft = std::make_unique<MethodCallExpression>(std::move(lft), std::move(segment), std::move(params_opt));
        break;
      }
      // field access
...
std::unique_ptr<LiteralExpression> Parser::parseLiteralExpression() {
  ...
  case TokenType::kCharLiteral: {
    ...
    while(lexeme[l] != '\'') ++l;
    char ch = lexeme[l + 1];
    return std::make_unique<LiteralExpression>(Prime::kChar, ch);
  }
...
std::unique_ptr<Conditions> Parser::parseConditions() {
  ...
  if(dynamic_cast<StructExpression*>(expr.get())) {
    ...
    REPORT_FAILURE_AND_RETURN("Condition should not be a struct expression");
  }
  tracker.commit();
