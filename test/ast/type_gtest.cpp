...
class AstTypeTest : public ::testing::Test {
  protected:
    ...
    void SetUp() override {
      ...
      auto bool_ref_mut = pool.make_type<RefType>(bool_type, false);
...
TEST_F(AstTypeTest, StringSliceAndByteSliceAsSameType) {
  ...
  auto u8_ref_slice = pool.make_raw_type<RefType>(u8_slice, false);
  ...
  auto char_ref_slice = pool.make_raw_type<RefType>(char_slice, false);
